...
male_fert_filt <- read.csv('data/male_fertility.csv') %>% 
...
anova(male_fec_test, type = "III", ddf = "Kenward-Roger") %>% broom::tidy() %>% 
  as_tibble() %>% 
  kable(digits = 3, 
...
anova(male_fec_coevo, type = "III", ddf = "Kenward-Roger") %>% broom::tidy() %>% 
  as_tibble() %>% 
  kable(digits = 3, 
        caption = 'Type III Analysis of Variance Table with Kenward-Roger`s method') %>% 
  kable_styling(full_width = FALSE)
...
car::Anova(hatch2, type = "3") %>% broom::tidy() %>% 
  as_tibble() %>% 
  kable(digits = 3, 
...
car::Anova(ster_md, type = "3") %>% broom::tidy() %>% 
  as_tibble() %>% 
  kable(digits = 3, 
        caption = 'Analysis of Deviance Table (Type III Wald chisquare tests)') %>% 
  kable_styling(full_width = FALSE)
...
### >>>> 
```{r}
car::Anova(viability_m1, type = "3") %>% broom::tidy() %>% 
  as_tibble() %>% 
  kable(digits = 3, 
        caption = 'Analysis of Deviance Table (Type III Wald chisquare tests)') %>% 
  kable_styling(full_width = FALSE)
...
sperm_vib_norm <- emmeans(viability_m1, ~ mito * nuclear * age, type = 'response') %>% as_tibble() %>% 
  ...
  geom_line(aes(group = mito, colour = mito), position = position_dodge(width = .5)) +
  # geom_errorbar(aes(ymin = asymp.LCL, ymax = asymp.UCL), 
  #               width = .25, position = position_dodge(width = .65)) +
  geom_point(size = 3, pch = 21, position = position_dodge(width = .5)) +
...
### >>> Results {.tabset}
```{r}
anova(vib_diff_test, type = "III", ddf = "Kenward-Roger") %>% broom::tidy() %>% 
  as_tibble() %>% 
  kable(digits = 3, 
        caption = 'Type III Analysis of Variance Table with Kenward-Roger`s method') %>% 
  kable_styling(full_width = FALSE)
